...
export function PostPurchaseInvoiceForm({ action, id }: { action: PurchasingAction; id: string }) {
...
export function CancelPurchaseInvoiceForm({
  action,
  id,
}: {
...
export function ReversePurchaseInvoiceForm({
  action,
  id,
}: {
  action: PurchasingAction;
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-3">
      ...
      <button
        ...
        disabled={pending}
      >
        {pending ? "Reversing..." : "Reverse posted invoice"}
      </button>
      {state.message && (
        <span className="text-sm" role="status">
          {state.message}
