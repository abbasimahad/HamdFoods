  ...
  return (
    <ResponsiveContainer>
      <PageHeader
        ...
        description="Immutable material transaction provenance from the existing production-batch engine."
      />
      <Card className="grid gap-3 p-5 sm:grid-cols-2">
        <Fact l="Batch" v={tx.batchNumber} />
        <Fact l="Type / status" v={`${tx.transactionType} · ${tx.status}`} />
        <Fact l="Material" v={`${tx.line.itemCode} · ${tx.line.itemName}`} />
      ...
      <div className="mt-4 flex gap-2">
        <Link
          ...
          href={`/production/batches/${tx.productionBatchId}/materials`}
        >
        ...
        {tx.status === "DRAFT" && hasPermission(principal, "production.manage") ? (
          <Link
            className="rounded border px-4 py-2"
            href={`/production/material-issues/${tx.id}/edit`}
          >
            Edit
          </Link>
        ) : null}
