...
export default async function Page() {
  ...
  const batches = await new PrismaProductionTransactionWorkbench().eligibleBatches("material");
  return (
    <ResponsiveContainer>
      <PageHeader
        title="New Material Issue"
        description="Choose an eligible batch, then use its existing material transaction engine."
      />
        ...
        {batches.map((batch) => (
          <Card className="flex items-center justify-between gap-3 p-4" key={batch.id}>
            ...
            <Link
              ...
              href={`/production/batches/${batch.id}/materials?type=ISSUE`}
            >
              Select batch
            </Link>
          </Card>
