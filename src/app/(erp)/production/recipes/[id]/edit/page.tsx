import { notFound, redirect } from "next/navigation";
import { RecipeForm } from "@/components/production/recipe-form";
import { PageHeader } from "@/components/layout/page-header";
import { Card } from "@/components/ui/card";
import { ResponsiveContainer } from "@/components/ui/responsive-container";
import { requirePermission } from "@/server/auth/server-guards";
import { PrismaRecipeRepository } from "@/server/production/prisma-recipe-repository";
import { saveRecipeAction } from "../../actions";
export default async function EditRecipePage({ params }: { params: Promise<{ id: string }> }) {
  await requirePermission("production.manage");
  const repository = new PrismaRecipeRepository();
  const recipe = await repository.getRecipe((await params).id);
  if (!recipe) notFound();
  if (recipe.status !== "DRAFT") redirect(`/production/recipes/${recipe.id}`);
  const [items, units] = await Promise.all([
    repository.listCatalogItems(),
    repository.listRecipeUnits(),
  ]);
  return (
    <ResponsiveContainer>
      <PageHeader
        title={`Edit ${recipe.code} v${recipe.version}`}
        description="Only DRAFT formula versions can be changed."
      />
      <Card className="p-5">
        <RecipeForm action={saveRecipeAction} initial={recipe} items={items} units={units} />
      </Card>
    </ResponsiveContainer>
  );
}
